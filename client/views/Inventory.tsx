...
const Inventory = ({
  ...
  const sellItem = async (props: any): Promise<AxiosResponse> => {
    try {
      ...
      const sell = await axios
        .post(baseUrl + "/inventory/sell", {
          id_item: props.id,
          id_user: 0,
          amount: props.amount,
          coins: props.coins,
  ...
  const tradeIt = async (props: any): Promise<any> => {
    //let baseUrl: string = "http://localhost:3000"
    let baseUrl: string = "";

    let item: AxiosResponse = await axios.get(baseUrl + "/inventory/item", {
      params: { nr: props.id },
    });

    //alert('oo');
    let element: any = document.getElementById("overScreen2");
    //console.log(props);
  ...
  const handleTab = function (props: any) {
    try {
      ...
      if (props.action == "ctnItems") {
        setCtnItems("active");
      }
      if (props.action == "ctnProfile") {
        setCtnProfile("active");
      }
      if (props.action == "ctnSpells") {
        setCtnSpells("active");
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="Inventory centerView">
      <div
        className="btn-close pointer btnCloseOverScreen"
        onClick={closeOverScreen}
      >
      ...
      <div className="cflx mgB ctnProfile">
        ...
        <div className="fr7">
          ...
          <div className="cflx">
            ...
            <p className="txt-gray mgL2">
              {" "}
              XP for next level: <b>999</b>
            </p>
          </div>
          <div className="cflx">
            ...
            <div className="cflx center-flex">
              ...
              <img
                src="../imagens/ic_coin.png"
                alt="Coin"
                className="mgL hide_resp"
                height="20"
              />
            </div>
          </div>
      ...
      <div className="nav-horizontal">
        <button
          className={ctnProfile}
          onClick={() => {
            handleTab({ action: "ctnProfile" });
          }}
          type="button"
        >
          Equipments
      ...
      >
        {Object.keys(items.data.data).map((d) => {
          if (items.data.data[d].type == "1" && items.data.data[d].amount > 0) {
            return (
              <div
                className="pp_square_black cflx gap-10 center-flex"
                key={items.data.data[d].id}
              >
                <div className="fr1">
                  <img
                    src={"./imagens/" + items.data.data[d].image}
                    height="40"
                  />
                </div>
                <div className="fr1 center mgR">
                  <div className="badge2">{items.data.data[d].amount} x</div>
                </div>
                ...
                <div className="fr2 gap-10 cflx justify-right">
                  ...
                  >
                    {" "}
                    <i className="fa fa-check-circle"></i> Use
                  </button>
                  <button
                    className="btn btn-main btn-sm"
                    onClick={() => {
                      tradeIt({ id: items.data.data[d].id, props: props });
                    }}
                  >
                    {" "}
                    <i className="fa fa-compass"></i> Trade
                  </button>
                  <button
      ...
      >
        <div className="row">
          {Object.keys(items.data.data).map((d) => {
            if (
              items.data.data[d].type == "3" &&
              items.data.data[d].amount > 0
            ) {
              return (
                    ...
                    <div className="cflx gap-15">
                      ...
                      <div className="fr4">
                        <div className="cflxss">
                          <div className="fr1">
                            <h3 className="title mg0">
                              {items.data.data[d].title}
                            </h3>
                            <p
                              className="paragraph txt-white"
                              style={{ height: "auto" }}
                            >
                              {items.data.data[d].description}
                            </p>
                          </div>
